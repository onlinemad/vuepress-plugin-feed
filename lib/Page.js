...
const files = fs.readdirSync('./global-components/')
files.map((file) => {
	if (file.split('.').pop() === 'vue') {
		let parsed = compiler.parseComponent(fs.readFileSync(`./global-components/${file}`, 'utf-8'))
		let component = Object.assign(
			{ template: parsed.template.content },
			parsed.script ? eval(parsed.script.content.replace('export default', 'module.exports =')) : {}
		)
			...
			if ( this._strippedContent )
			{
				...
				*/
				...
				return new Promise((resolve) => {
					ssr.createRenderer().renderToString(new Vue({
